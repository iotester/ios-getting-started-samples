...
3. Run strip-frameworks script:
    ...
    + In Build Phases, add a run script phase (if not present) and add the below line to run the strip-frameworks script.
      bash "${BUILT_PRODUCTS_DIR}/${FRAMEWORKS_FOLDER_PATH}/AdobeCreativeSDKCore.framework/strip-frameworks.sh" <br /><br /><img style="border: 1px solid #ccc;" src="strip-frameworks.png" /><br /><br />
